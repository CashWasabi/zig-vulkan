...
pub fn main() !void {
    ...
    // =====================
    var vertices: [3]Vertex = [_]Vertex{
        .{
            .pos = .{ .x = 0.0, .y = -0.5 },
    ...
    if (c.vkCreateBuffer(logical_device, &vertex_buffer_create_info, null, &vertex_buffer) != c.VK_SUCCESS) {
        @panic("failed to create vertex buffer!");
    }
    defer c.vkDestroyBuffer(logical_device, vertex_buffer, null);

    var mem_requirements: c.VkMemoryRequirements = .{};
    ...
    if (c.vkAllocateMemory(logical_device, &alloc_info, null, &vertex_buffer_memory) != c.VK_SUCCESS) {
        @panic("failed to allocate vertex buffer memory!");
    }
    defer c.vkFreeMemory(logical_device, vertex_buffer_memory, null);

    if (c.vkBindBufferMemory(logical_device, vertex_buffer, vertex_buffer_memory, 0) != c.VK_SUCCESS) {
        @panic("failed to bind vertex buffer memory!");
    }

    var data: ?*anyopaque = null;
    if (c.vkMapMemory(logical_device, vertex_buffer_memory, 0, vertex_buffer_create_info.size, 0, &data) != c.VK_SUCCESS) {
        @panic("failed to map vertex buffer memory!");
    }
    const dst: [*]Vertex = @ptrCast(@alignCast(data));
