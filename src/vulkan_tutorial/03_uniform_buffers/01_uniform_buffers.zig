...
pub fn main() !void {
    ...
    const queue_familiy_indices = try findQueueFamilies(allocator, physical_device, surface);
    ...
    const graphics_queue_create_info: c.VkDeviceQueueCreateInfo = .{
        ...
        .queueFamilyIndex = queue_familiy_indices.graphics_family.?,
        .queueCount = 1,
    ...
    const present_queue_create_info: c.VkDeviceQueueCreateInfo = .{
        .sType = c.VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO,
        .queueFamilyIndex = queue_familiy_indices.present_family.?,
        .queueCount = 1,
    ...
    c.vkGetDeviceQueue(logical_device, queue_familiy_indices.graphics_family.?, 0, &graphics_queue);

    var present_queue: c.VkQueue = null;
    c.vkGetDeviceQueue(logical_device, queue_familiy_indices.present_family.?, 0, &present_queue);
    ...
    queue_family_indices_slice[0] = queue_familiy_indices.graphics_family.?;
    queue_family_indices_slice[1] = queue_familiy_indices.present_family.?;
    ...
    if (c.vkCreateDescriptorSetLayout(logical_device, &layout_info, null, &descriptor_set_layout) != c.VK_SUCCESS) {
    ...
    // CREATE INDEX BUFFER
    // =====================
    var vertex_indices = [_]VertexIndex{ 0, 1, 2, 2, 3, 0 };
    ...
    // =======================
    var ubo: UniformBufferObject = .default;
    ...
    if (c.vkCreateDescriptorPool(logical_device, &descriptor_pool_info, null, &descriptor_pool) != c.VK_SUCCESS) {
    ...
    if (c.vkAllocateDescriptorSets(logical_device, &descriptor_set_alloc_info, &descriptor_sets) != c.VK_SUCCESS) {
    ...
    for (0..MAX_FRAMES_IN_FLIGHT) |i| {
        if (c.vkCreateSemaphore(logical_device, &semaphore_create_info, null, &image_available_semaphores[i]) != c.VK_SUCCESS or
            c.vkCreateSemaphore(logical_device, &semaphore_create_info, null, &render_finished_semaphores[i]) != c.VK_SUCCESS or
            c.vkCreateFence(logical_device, &fence_create_info, null, &in_flight_fences[i]) != c.VK_SUCCESS)
        {
    ...
    while (!G_SHOULD_EXIT) {
        ...
        // UPDATE UNIFORM BUFFER
        {
            ...
            ubo.model = mat4Rotation(
                time_s * std.math.degreesToRadians(90),
                .{ .x = 0.0, .y = 0.0, .z = 1.0 },
            );
            ubo.view = mat4LookAt(
                .{ .x = 2.0, .y = 2.0, .z = 2.0 },
            ...
            ubo.proj = mat4Perspective(
                std.math.degreesToRadians(45),
                @as(f32, @floatFromInt(swap_chain_extent.width)) / @as(f32, @floatFromInt(swap_chain_extent.height)),
                0.1,
                10.0,
            );

            // ubo.proj[1][1] *= -1; // opengl specific????

            // update UBO buffer
            const dst: [*]u8 = @ptrCast(uniform_buffers_mapped[current_frame]);
            const src: [*]const u8 = @ptrCast(&ubo);
            @memcpy(dst[0..@sizeOf(UniformBufferObject)], src[0..@sizeOf(UniformBufferObject)]);
        }

        if (c.vkResetFences(logical_device, 1, &in_flight_fences[current_frame]) != c.VK_SUCCESS) {
            @panic("failed while trying to reset fences in main loop.");
        }
        if (c.vkResetCommandBuffer(command_buffers[current_frame], 0) != c.VK_SUCCESS) {
        ...
        var wait_semaphores = [_]c.VkSemaphore{image_available_semaphores[current_frame]};
        var wait_stages = [_]c.VkPipelineStageFlags{c.VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT};
        var signal_semaphores = [_]c.VkSemaphore{render_finished_semaphores[current_frame]};
